...
static int pcapFindSnap(const uint8_t *frm, uint16_t len) {
...
bool fast_pcap_parse(const char *path, CrackHandshake &hs, int *reason) {
    ...
    while (f.available() > 16) {
        ...
        if (!pcapRead32(f,ts)||!pcapRead32(f,tu)||!pcapRead32(f,incl)||!pcapRead32(f,orig)) break;
        ...
        int snap = pcapFindSnap(frm, flen);
