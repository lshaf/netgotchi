...
public:
  ...
  // DLDO1 feeds the SY7088 boost converter for the LCD LEDs. Empirically the
  // SY7088 only attenuates between val 19 (≈2.4V, faint glow) and val 23
  // (≈2.8V, full bright); anything ≥23 looks identical. Compress user 1–100%
  // into val 19–23 for a perceptually-even slider.
  ...
  void setBacklight(uint8_t pct) {
    ...
    if (pct > 100) pct = 100;
    uint8_t val = (uint8_t)(19 + ((uint16_t)pct * 4) / 100);
    _write(0x99, val);
    _bitOn(0x90, 0x80);
  }
